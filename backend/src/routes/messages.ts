...
messages.post("/search", zValidator("json", searchMessagesRequestSchema), async (c) => {
  try {
    ...
    const matchMap = new Map<string, { id: string; score: number; matchedField: "content" | "transcription" | "description"; similarity?: number; rank?: number; createdAt: string }>();
      ...
      .map(r => {
        ...
        const formattedMessage = {
          ...
          linkPreview: msg.linkPreviewUrl ? {
            ...
            ...(msg.linkPreviewUrl && !msg.linkPreviewTitle ? { title: "" } : {})
          } : null,
          user: msg.user ? {
            id: msg.user.id,
            name: msg.user.name,
